...
function Get-V4BOverlayFactValues($Product, [string]$Slot) {
    ...
    $priority = @(
        'verified_dimensions',
        'verified_resistance_levels',
        'verified_models',
        'verified_sizes',
        'verified_materials',
        'verified_accessories',
        'verified_colors',
        'verified_bundle_contents',
        'verified_features'
    )
    $values = @()
...
function Get-V4BVerifiedOverlayContent($Product, [string]$Slot) {
    ...
    if ([string]::IsNullOrWhiteSpace($title) -or $title -eq '商品') {
        $title = if ($Slot -eq 'main') { '商品展示' } else { '商品資訊' }
    }
    $facts = @(Get-V4BOverlayFactValues $Product $Slot)
    $secondary = ''
    if ($facts.Count -gt 0) {
        $secondary = $facts -join ' ・ '
    }
    else {
    ...
    return [pscustomobject]@{
        title = Convert-ToTaiwanCommerceTextV4B ([string]$title)
        secondary = Convert-ToTaiwanCommerceTextV4B ([string]$secondary)
        facts = [string[]]$facts
        deterministic = $true
    }
}
...
function Set-V4BPendingOverlay($Product, [string]$Slot) {
    ...
    if ($null -ne $slotPlan -and [bool](Get-V4A1Property $slotPlan 'text_shield_required' $false)) {
        $script:V4BPendingOverlay = [pscustomobject]@{
            product = $Product
            slot = $Slot
            product_id = [string](Get-V4A1Property $Product 'product_id' '')
...
function Get-V4BInstalledOverlayFontName {
    ...
    foreach ($name in @('Microsoft JhengHei UI','Microsoft JhengHei','Noto Sans CJK TC','Arial Unicode MS','Arial')) {
        $font = $null
        try {
            $font = New-Object Drawing.Font $name,12,[Drawing.FontStyle]::Regular,[Drawing.GraphicsUnit]::Pixel
...
function New-V4BFittedFont($Graphics, [string]$FontName, [string]$Text, [float]$StartSize, [float]$MinSize, [float]$MaxWidth, [bool]$Bold) {
    ...
    return (New-Object Drawing.Font $FontName,$MinSize,$style,[Drawing.GraphicsUnit]::Pixel)
}
...
function Add-V4BVerifiedOverlay([string]$Path, $Content) {
    ...
    try {
        ...
        $cardHeight = [Math]::Max(132.0, [Math]::Round($height * 0.17))
        $cardY = $height - $cardHeight
        $paddingX = [Math]::Max(28.0, [Math]::Round($width * 0.04))
        $maxTextWidth = $width - 2.0 * $paddingX

        $background = New-Object Drawing.SolidBrush ([Drawing.Color]::FromArgb(205,18,18,18))
        $graphics.FillRectangle($background,0,$cardY,$width,$cardHeight)
        $borderPen = New-Object Drawing.Pen ([Drawing.Color]::FromArgb(120,255,255,255)),1
        $graphics.DrawLine($borderPen,0,$cardY,$width,$cardY)

        $fontName = Get-V4BInstalledOverlayFontName
        $title = [string](Get-V4A1Property $Content 'title' '')
        $secondary = [string](Get-V4A1Property $Content 'secondary' '')
        ...
        $secondaryBrush = New-Object Drawing.SolidBrush ([Drawing.Color]::FromArgb(235,235,235))

        $titleY = $cardY + [Math]::Max(12.0,[Math]::Round($cardHeight * 0.13))
        $graphics.DrawString($title,$titleFont,$titleBrush,[float]$paddingX,[float]$titleY)
        if (-not [string]::IsNullOrWhiteSpace($secondary)) {
            $secondaryY = $titleY + $titleFont.Height + [Math]::Max(7.0,[Math]::Round($cardHeight * 0.05))
            $graphics.DrawString($secondary,$secondaryFont,$secondaryBrush,[float]$paddingX,[float]$secondaryY)
        }

        $temp = $Path + '.v4b_overlay.jpg'
...
function Get-PromptV2([string]$Slot, $ProductOrName) {
    ...
    $base = & $script:V4BOverlayPromptBase $Slot $ProductOrName
    $shielded = Set-V4BPendingOverlay $product $Slot
    if ($shielded) {
        $base += "`n[V4-B 程式化驗證文字覆蓋]`n本張 TinySnow 階段只製作乾淨、忠實的商品視覺。不要生成任何可辨識文字、數字、單位、徽章、icon 字母或標題；請保留畫面下方約 17% 的乾淨空間或簡潔背景。生成完成後，Windows 程式會自行疊加共同已驗證的台灣繁體文字，因此不要自行補任何文字。"
    }
    return $base
}
...
function Get-CompactTransportPromptV2([string]$Slot, $ProductOrName) {
    $product = Get-V4BResolvedProduct $ProductOrName
    $base = & $script:V4BOverlayCompactBase $Slot $ProductOrName
    $shielded = Set-V4BPendingOverlay $product $Slot
    if ($shielded) {
        $base += ' 本張 TinySnow 只輸出乾淨商品視覺，不生成任何可讀文字、數字、單位或帶字母 icon；下方留乾淨空間，驗證文字由 Windows 程式後處理疊加。'
    }
    return $base
}

function Convert-ToFinalJpegV2([string]$Source, [string]$Target) {
    $result = & $script:V4BFinalJpegBase $Source $Target
    $pending = $script:V4BPendingOverlay
    $script:V4BPendingOverlay = $null
    if ($null -ne $pending) {
        Add-V4BVerifiedOverlay $Target $pending.content
