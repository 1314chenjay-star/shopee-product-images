...
function Show-Selected {
    ...
    catch { Write-Host '目前商品：尚未選擇' -ForegroundColor Yellow }
}
    ...
    try {
        switch ($choice) {
            ...
            '4' {
                ...
                Save-TinySnowConfig $config
                Write-Host ''
                Write-Host "已選擇：$number. [$($product.product_id)] $($product.product_name)" -ForegroundColor Green
            ...
            '5' {
                ...
                Write-Host ''
                Write-Host "原圖檢查完成：$(@($result.downloaded).Count) 張可用" -ForegroundColor Green
                Write-Host "下載失敗：$(@($result.failed_urls).Count) 張"
                Write-Host "檢查結果資料夾：$($result.folder)"
            ...
            '8' { Open-Folder (Join-Path $root 'workspace\final_images') }
            '9' {
                $product = Get-SelectedProduct
