...
def main():
 ...
 qseq={int(x['sequence']) for x in queue};pseq={int(x['sequence']) for x in pres}
 if len(pseq)!=len(pres):raise RuntimeError('Duplicate sequence in frozen preservation results')
 outside=sorted(pseq-qseq)
 if outside:raise RuntimeError(f'Preservation result leaked outside semantic queue: {outside[:10]}')
 remaining=[]
 for q in queue:
  seq=int(q['sequence'])
  if seq in pseq:continue
  ...
  remaining.append(r)
 if len(remaining)!=len(queue)-len(pres):raise RuntimeError('Future bridge reconciliation mismatch')
