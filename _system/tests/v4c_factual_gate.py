...
def norm_type(c): return str(c.get("type") or "unknown").strip().lower()
...
def is_high_risk(c):
    hay=f"{norm_type(c)}|{str(c.get('origin') or '').lower()}|{norm_value(c).lower()}"
    if norm_type(c)=="risk_field" or norm_type(c).startswith("risk_"): return True
...
def _collect_variant(obj,out):
    ...
    for k in VARIANT_KEYS:
        ...
        if isinstance(v,list):
            for x in v:
                if x not in (None,""): out.add(f"{k}:{x}")
        else: out.add(f"{k}:{v}")
...
def classify_claim(c,bucket):
    ...
    if status in {"CONFLICT","FACT_CONFLICT"}: return base_fact(c,"FACT_CONFLICT","EXPLICIT_CONFLICT_IN_DURABLE_EVIDENCE")
    if bucket=="verified":
        if status!="VERIFIED_SOURCE": return base_fact(c,"FACT_FORBIDDEN","NOT_VERIFIED_SOURCE")
        if usage in {"","NONE"}: return base_fact(c,"FACT_FORBIDDEN","VERIFIED_SOURCE_NOT_ALLOWED_FOR_USAGE")
        if not evidence_present(c): return base_fact(c,"FACT_FORBIDDEN","VERIFIED_SOURCE_WITHOUT_DURABLE_EVIDENCE")
        return base_fact(c,"FACT_VERIFIED","EXISTING_VERIFIED_SOURCE_EVIDENCE")
    if status=="VERIFIED_SOURCE" and usage not in {"","NONE"} and evidence_present(c): return base_fact(c,"FACT_CONFLICT","CLAIM_BUCKET_STATUS_CONFLICT")
    if usage!="NONE": return base_fact(c,"FACT_CONFLICT","UNKNOWN_ALLOWED_USAGE_VIOLATION")
    if is_high_risk(c): return base_fact(c,"FACT_FORBIDDEN","HIGH_RISK_CLAIM_WITHOUT_VERIFIED_SOURCE")
    return base_fact(c,"FACT_UNKNOWN","UNKNOWN_ALLOWED_USAGE_NONE")

def fact_key(f): return (str(f.get("claim_type") or "").lower(),tuple(f.get("variant_scope") or []))

def detect_verified_conflicts(facts):
    verified=[f for f in facts if f["classification"]=="FACT_VERIFIED"]
    by_scope=defaultdict(list)
    for f in verified: by_scope[fact_key(f)].append(f)
    ...
    for items in by_scope.values():
        ...
        if len(vals)>1:
            for x in items: conflict_ids.add(x["fact_id"])
    by_type=defaultdict(list)
    for f in verified: by_type[str(f.get("claim_type") or "").lower()].append(f)
    for items in by_type.values():
        scoped=[x for x in items if x.get("variant_scope")]; unscoped=[x for x in items if not x.get("variant_scope")]
        if not scoped or not unscoped: continue
        ...
        for u in unscoped:
            ...
            if any(sv!=uv for sv in scoped_vals): conflict_ids.add(u["fact_id"]); variant_conflict_ids.add(u["fact_id"])
    for f in facts:
...
def exact_text(f):
    val=f.get("value")
    val=json.dumps(val,ensure_ascii=False,sort_keys=True,separators=(",",":")) if isinstance(val,(dict,list)) else str(val if val is not None else "")
...
def gate_image(rec):
    if rec.get("final_status")!="PARTIAL_SAFE": raise RuntimeError(f"Non-PARTIAL_SAFE entered V4-C3: seq {rec.get('sequence')}")
    seen=set(); facts=[]
    ...
    if conflicts: elig="BLOCK_FACTUAL"
    elif not verified: elig="HOLD_FACTUAL"
    elif unknown or forbidden: elig="ELIGIBLE_PARTIAL"
    else: elig="ELIGIBLE_SAFE"
    return {"schema_version":SCHEMA,"sequence":int(rec["sequence"]),"product_id":str(rec.get("product_id") or ""),"sha256":str(rec.get("sha256") or ""),"source_final_status":"PARTIAL_SAFE","evidence_source":rec.get("evidence_source"),"verified_facts":verified,"unknown_facts":unknown,"conflict_facts":conflicts,"forbidden_facts":forbidden,"generation_safe_text":[exact_text(x) for x in verified],"generation_forbidden_text":[exact_text(x) for x in unknown+conflicts+forbidden],"generation_safe_fact_ids":[x["fact_id"] for x in verified],"generation_eligibility":elig,"variant_conflicts":variant_conflicts,"downstream_generation_allowed":elig in {"ELIGIBLE_SAFE","ELIGIBLE_PARTIAL"},"api_flags":zero_flags()}

def plan(a):
    rows=read_jsonl(a.canonical)
    if len(rows)!=EXPECTED_TOTAL: raise RuntimeError(f"Expected canonical {EXPECTED_TOTAL}, got {len(rows)}")
    ...
    if dict(counts)!=expected: raise RuntimeError(f"Canonical frozen counts changed: {dict(counts)}")
    targets=sorted([r for r in rows if r.get("final_status")=="PARTIAL_SAFE"],key=lambda x:int(x["sequence"]))
    if len(targets)!=EXPECTED_PARTIAL: raise RuntimeError("PARTIAL_SAFE target mismatch")
    if any((r.get("downstream_eligibility") or {}).get("downstream_generation_allowed") is not True for r in targets): raise RuntimeError("Canonical PARTIAL_SAFE downstream eligibility changed")
...
def process(a):
    manifest=read_jsonl(a.manifest); existing=read_jsonl(a.progress) if Path(a.progress).exists() else []
    byseq={int(r["sequence"]):r for r in existing}; before=len(byseq); processed=0
    for rec in manifest:
        seq=int(rec["sequence"])
        if seq in byseq: continue
        if a.max_items is not None and processed>=a.max_items: break
        byseq[seq]=gate_image(rec); processed+=1
    write_jsonl(a.progress,[byseq[k] for k in sorted(byseq)])
...
def validate_rows(rows):
    for r in rows:
        if r.get("source_final_status")!="PARTIAL_SAFE": raise RuntimeError(f"Non-PARTIAL_SAFE output seq {r.get('sequence')}")
        if r.get("generation_eligibility") not in ELIGIBILITY: raise RuntimeError(f"Bad eligibility seq {r.get('sequence')}")
        ...
        if set(r.get("generation_safe_fact_ids") or [])!={x.get("fact_id") for x in verified}: raise RuntimeError(f"Safe fact id mismatch seq {r.get('sequence')}")
        if len(r.get("generation_safe_text") or [])!=len(verified): raise RuntimeError(f"Safe text not verified-only seq {r.get('sequence')}")
        for f in verified:
            if f.get("source_status")!="VERIFIED_SOURCE" or f.get("allowed_usage") in {None,"","NONE"} or not f.get("evidence"): raise RuntimeError(f"FACT_VERIFIED without durable VERIFIED_SOURCE seq {r.get('sequence')}")
        for f in unknown:
            if f.get("allowed_usage")!="NONE": raise RuntimeError(f"FACT_UNKNOWN allowed_usage violation seq {r.get('sequence')}")
            if is_high_risk({"type":f.get("claim_type"),"value":f.get("value"),"origin":f.get("origin")}): raise RuntimeError(f"High risk claim left FACT_UNKNOWN seq {r.get('sequence')}")
        for f in forbidden:
            if f.get("reason")=="HIGH_RISK_CLAIM_WITHOUT_VERIFIED_SOURCE" and f.get("allowed_usage")!="NONE": raise RuntimeError(f"Forbidden high-risk usage violation seq {r.get('sequence')}")
        if conflicts and r.get("generation_eligibility")!="BLOCK_FACTUAL": raise RuntimeError(f"Conflict did not block seq {r.get('sequence')}")
        if not verified and r.get("generation_eligibility") not in {"HOLD_FACTUAL","BLOCK_FACTUAL"}: raise RuntimeError(f"No verified fact but eligible seq {r.get('sequence')}")
        if any((r.get("api_flags") or {}).values()): raise RuntimeError(f"Forbidden API/retest flag true seq {r.get('sequence')}")

def synthetic_policy_tests():
    ev=[{"source":"synthetic","text":"explicit"}]
    assert classify_claim({"claim_id":"v","type":"material","value":"cotton","status":"VERIFIED_SOURCE","allowed_usage":"FACT_EXACT_ONLY","evidence":ev},"verified")["classification"]=="FACT_VERIFIED"
    assert classify_claim({"claim_id":"u","type":"title_text","value":"x","status":"UNKNOWN","allowed_usage":"NONE","evidence":[]},"unknown")["classification"]=="FACT_UNKNOWN"
    assert classify_claim({"claim_id":"h","type":"material","value":"cotton","status":"UNKNOWN","allowed_usage":"NONE","evidence":[]},"unknown")["classification"]=="FACT_FORBIDDEN"
    ...
    facts=[a,b]; assert detect_verified_conflicts(facts)==0 and all(x["classification"]=="FACT_VERIFIED" for x in facts)
    ...
    facts=[a,b,u]; assert detect_verified_conflicts(facts)==1 and u["classification"]=="FACT_CONFLICT"

def self_test(_a=None): synthetic_policy_tests(); print("V4_C3_SELF_TEST=PASS")
...
def validate_canary(a):
    ...
    if len(manifest)!=a.expected or len(out)!=a.expected: raise RuntimeError("Canary input/output reconciliation failed")
    if {int(x["sequence"]) for x in manifest}!={int(x["sequence"]) for x in out}: raise RuntimeError("Canary sequence reconciliation failed")
    if seed.get("covered_after_run")!=a.seed: raise RuntimeError("Canary seed checkpoint mismatch")
    if resume.get("checkpoint_existing_terminal")!=a.seed or resume.get("covered_after_run")!=a.expected: raise RuntimeError("Canary resume checkpoint mismatch")
...
def aggregate_product(rows):
    ...
    for r in rows: byprod[str(r.get("product_id") or "")].append(r)
    products=[]
    for pid,items in sorted(byprod.items()):
        buckets=[{}, {}, {}, {}]
        for r in items:
            for dest,key in zip(buckets,("verified_facts","unknown_facts","conflict_facts","forbidden_facts")):
                for fact in r.get(key) or []:
                    ...
                    dest[stable]=fact
        verified=list(buckets[0].values()); unknown=list(buckets[1].values()); conflicts=list(buckets[2].values()); forbidden=list(buckets[3].values())
        if conflicts or any(x.get("generation_eligibility")=="BLOCK_FACTUAL" for x in items): elig="BLOCK_FACTUAL"
        elif not verified: elig="HOLD_FACTUAL"
        elif all(x.get("generation_eligibility")=="ELIGIBLE_SAFE" for x in items): elig="ELIGIBLE_SAFE"
        else: elig="ELIGIBLE_PARTIAL"
        products.append({"schema_version":"v4c3.factual-gate-product.1","product_id":pid,"sequences":[int(x["sequence"]) for x in sorted(items,key=lambda z:int(z["sequence"]))],"verified_facts":verified,"unknown_facts":unknown,"conflict_facts":conflicts,"forbidden_facts":forbidden,"generation_safe_text":[exact_text(x) for x in verified],"generation_forbidden_text":[exact_text(x) for x in unknown+conflicts+forbidden],"generation_eligibility":elig,"downstream_generation_allowed":elig in {"ELIGIBLE_SAFE","ELIGIBLE_PARTIAL"}})
    return products

def aggregate(a):
    target=read_jsonl(a.target_manifest); rows=sorted(read_jsonl(a.canary_progress)+read_jsonl(a.full_progress),key=lambda x:int(x["sequence"]))
    if len(target)!=EXPECTED_PARTIAL or len(rows)!=EXPECTED_PARTIAL: raise RuntimeError("Full V4-C3 reconciliation count mismatch")
    if len({int(x["sequence"]) for x in rows})!=EXPECTED_PARTIAL: raise RuntimeError("Duplicate/missing image output sequence")
    if {int(x["sequence"]) for x in rows}!={int(x["sequence"]) for x in target}: raise RuntimeError("Target/output sequence mismatch")
    validate_rows(rows); products=aggregate_product(rows)
    write_jsonl(a.image_out,rows); write_jsonl(a.product_out,products)
    queue=[r for r in rows if r.get("downstream_generation_allowed")]; write_jsonl(a.queue_out,queue)
    ec=Counter(r["generation_eligibility"] for r in rows)
    s={"schema_version":"v4c3.factual-gate-summary.1","passed":True,"input_partial_safe":EXPECTED_PARTIAL,"eligibility_counts":dict(sorted(ec.items())),"verified_fact_count":sum(len(r.get("verified_facts") or []) for r in rows),"unknown_fact_count":sum(len(r.get("unknown_facts") or []) for r in rows),"conflict_fact_count":sum(len(r.get("conflict_facts") or []) for r in rows),"forbidden_fact_count":sum(len(r.get("forbidden_facts") or []) for r in rows),"variant_conflicts":sum(int(r.get("variant_conflicts") or 0) for r in rows),"downstream_generation_eligible_count":len(queue),"product_count":len(products),"api_flags":zero_flags()}
    write_json(a.summary,s); print(json.dumps(s,ensure_ascii=False,sort_keys=True))

def validate_final(a):
    images=read_jsonl(a.images); products=read_jsonl(a.products); queue=read_jsonl(a.queue); s=json.loads(Path(a.summary).read_text(encoding="utf-8-sig"))
    if len(images)!=EXPECTED_PARTIAL: raise RuntimeError("Final image gate count mismatch")
    validate_rows(images)
    if len(queue)!=sum(1 for x in images if x.get("downstream_generation_allowed")): raise RuntimeError("Generation queue reconciliation failed")
    expected={int(x["sequence"]) for x in images if x.get("generation_eligibility") in {"ELIGIBLE_SAFE","ELIGIBLE_PARTIAL"}}
    if {int(x["sequence"]) for x in queue}!=expected: raise RuntimeError("Generation queue contains ineligible/missing records")
    for p in products:
        if p.get("generation_eligibility") in {"HOLD_FACTUAL","BLOCK_FACTUAL"} and p.get("downstream_generation_allowed") is not False: raise RuntimeError(f"Ineligible product allowed: {p.get('product_id')}")
    if any((s.get("api_flags") or {}).values()): raise RuntimeError("Forbidden API/retest flag true in summary")
