...
class ArtifactBatch:
    def __init__(self,row,repo,token,cache_dir):
        self.batch=row['artifact_batch'];self.artifact_name=row['artifact_name'];self.artifact_id=int(row['artifact_id'])
        self.zip_path=Path(cache_dir)/(self.artifact_name+'.zip')
        self.zip_path,self.downloaded=download_artifact(repo,self.artifact_id,token,self.zip_path)
        ...
        self.manifest_by_seq={int(x['sequence']):x for x in raw if x.get('sequence') is not None}
...
def process(a):
    ...
    target=len(manifest) if a.max_items<=0 else min(len(manifest),a.max_items);selected=manifest[:target]
    batches={};models=None;processed=skipped=0
    repo=a.repository or os.environ.get('GITHUB_REPOSITORY','');token=os.environ.get('GITHUB_TOKEN','') or os.environ.get('GH_TOKEN','')
    ...
    for row in selected:
        ...
        b=row['artifact_batch']
        if b not in batches:batches[b]=ArtifactBatch(row,repo,token,a.artifact_cache)
...
def summarize(rows):
    ...
    return {'target':len(rows),'artifact_found':sum(bool(r.get('artifact_found')) for r in rows),'artifact_not_found':sum(bool(r.get('artifact_not_found')) for r in rows),
            ...
            'hold_remaining':term.get('HOLD',0),'block':term.get('BLOCK',0),'pass':term.get('PASS',0),'remaining':0,
            'local_ocr_executed':sum(bool((r.get('flags') or {}).get('ocr_executed')) for r in rows),'semantic_actually_executed':0,
            'api_flags':{'github_artifact_download_called':any(bool((r.get('flags') or {}).get('github_artifact_download_called')) for r in rows),
                         'source_fetch_called':False,'image_generation_called':False,'tiny_snow_api_called':False,'paid_api_called':False,'vision_api_called':False,
                         'semantic_inference_executed':False,'rerun_1378':False,'source_refetch_1144':False,'inventory_rebuilt':False,'completed_ocr_rerun':False,'completed_semantic_rerun':False}}


def aggregate(a):
    targets=read_jsonl(a.target_manifest);expected={int(r['sequence']) for r in targets}
    rows=read_jsonl(a.canary_progress)+read_jsonl(a.full_progress);by={}
    for r in rows:
        seq=int(r['sequence'])
