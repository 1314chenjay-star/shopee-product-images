...
if ($prompt575 -notmatch '圖示與單位硬限制' -or $prompt575 -notmatch '拉丁字母' -or $prompt575 -notmatch '單位縮寫') { throw 'Unit-icon hardening rule missing.' }
if ($prompt575 -notmatch '禁止重建、猜測或補全任何可讀字樣') { throw 'Final surface-text suppression rule missing.' }
...
if ($compact532 -notmatch '拉丁字母' -or $compact532 -notmatch '單位縮寫' -or $compact532 -notmatch '禁止重建、猜測或補全可讀字樣') { throw 'Compact final hardening missing.' }

Write-Host '[PASS] V4-A.2 exact text allowlist, generic unit-icon suppression, and unverified surface-marking hardening passed.' -ForegroundColor Green
