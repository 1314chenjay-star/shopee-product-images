...
$legacyRecords=@($ordered|Where-Object{$_.status -eq "LEGACY_DONE"});$legacySequenceCoverage=0
foreach($p in $legacyRecords){if($p.PSObject.Properties.Name -contains "legacy_sequences"){$legacySequenceCoverage += @($p.legacy_sequences).Count}elseif($p.PSObject.Properties.Name -contains "legacy_sequence"){$legacySequenceCoverage++}}
...
$summary=[ordered]@{phase=$Phase;inventory_count=$inventory.Count;shard_result_count=$seenResult.Count;legacy_done=$legacyRecords.Count;legacy_sequence_coverage=$legacySequenceCoverage;legacy_duplicate_occurrences=(900-$legacyRecords.Count);done=@($ordered|Where-Object{$_.status -eq "DONE"}).Count;url_duplicates=$urlDupes.Count;sha256_duplicates=$shaDupes.Count;failed=$failed.Count;semantic_pending=$pending.Count;hold=$hold.Count;block=$block.Count;terminal_count=@($ordered|Where-Object{$_.status -in @("LEGACY_DONE","DONE","URL_DUPLICATE","SHA_DUPLICATE","FAILED")}).Count;image_generation_called=$false;tiny_snow_api_called=$false;paid_api_called=$false}
