...
# Cosmetic stale build text in the old menu file must render as V4-A.2.1 without rewriting API-key input logic.
$rendered = (& { Write-Host 'Build: V4-A.1｜真實資料＋視覺數量鎖定版' -ForegroundColor Yellow } 6>&1 | Out-String)
if ($rendered -notmatch 'V4-A\.2\.1｜圖片文字穩定修正版') { throw 'Legacy V4-A.1 menu build label was not replaced with V4-A.2.1.' }
if ($rendered -match 'V4-A\.1｜真實資料') { throw 'Legacy V4-A.1 menu build label still rendered.' }

Write-Host '[PASS] V4-A.2.1 menu loader/UX regression: final guards survive duplicate legacy load, Enter-to-return is automatic, and the stale menu build label renders as V4-A.2.1.' -ForegroundColor Green
