...
while ($true) {
    ...
    try {
        switch ($choice) {
            ...
            '4' {
                $catalog = Get-Catalog
                Write-Host '方案 A：直接輸入商品ID'
                Write-Host '方案 B：輸入 L 顯示商品清單'
                $inputValue = Read-Host '請輸入商品ID或 L'

                if ($inputValue -eq 'L') {
                    $items = @($catalog.products)
                    for ($index = 0; $index -lt $items.Count; $index++) {
                        Write-Host ("{0}. [{1}] {2}" -f ($index + 1), $items[$index].product_id, $items[$index].product_name)
                    }

                    $number = Read-Host '輸入清單編號'
                    if ($number -notmatch '^\d+$' -or [int]$number -lt 1 -or [int]$number -gt $items.Count) {
                        throw '清單編號無效。'
                    }
                    $inputValue = $items[[int]$number - 1].product_id
                }

                $product = Select-ShopeeProduct $inputValue
                $count = @($product.image_urls).Count
                $mainImage = if ($count) { $product.image_urls[0] } else { '無' }
                $detailCount = [Math]::Max(0, $count - 1)

                Write-Host ''
                Write-Host "商品ID：$($product.product_id)"
                Write-Host "商品名稱：$($product.product_name)"
                Write-Host "原始圖片數：$count"
                Write-Host "主圖：$mainImage"
                Write-Host "詳情圖數：$detailCount"

                $confirm = Read-Host '輸入 Y 鎖定這件商品，其他鍵取消'
                if ($confirm -ne 'Y') {
                    Remove-Item (Join-Path $root 'workspace\selected_product.json') -Force -ErrorAction SilentlyContinue
                    Write-Host '已取消。'
                }
                else {
                    $config.selected_product_id = $product.product_id
                    Save-TinySnowConfig $config
                    Write-Host '已鎖定單一商品。' -ForegroundColor Green
                }
                Pause-Menu
            }

            '5' {
                $confirm = Read-Host '將依序生成主圖與4張詳情圖，可能消耗額度。輸入 START 開始'
                if ($confirm -eq 'START') {
                    ...
                    if ($result.complete) {
                        ...
                        Write-Host '尚未完成，已保存斷點；修正問題後可再次執行續跑。' -ForegroundColor Yellow
                    }
                }
                else {
            ...
            '6' {
                $product = Get-SelectedProduct
                $checkpoint = Get-Checkpoint $product.product_id
                Write-Host "商品ID：$($product.product_id)"
            ...
            '8' {
                $product = Get-SelectedProduct
                $zip = New-ProductZip $product.product_id
